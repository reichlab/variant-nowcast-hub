...
model_summary <- function(target_date, file_path_to_metadata = "./model-metadata", file_path_to_output = "./model-output/"){
  ...
  for(file in file_names){
    ...
    suppressWarnings(yml_data <- yaml.load_file(paste0(file_path_to_metadata, "/", file)))
    Model_creator[j] <- yml_data$team_name
    # reading in the model file
    model <- read_parquet(paste0(file_path_to_output, substr(file, 1, nchar(file) - 4),
                                 "/", target_date, "-", substr(file, 1, nchar(file) - 3), "parquet"))
  ...
  # reading the infomation into a txt file
  save_file_name = paste0("./model_output_summary_", target_date, ".txt")
