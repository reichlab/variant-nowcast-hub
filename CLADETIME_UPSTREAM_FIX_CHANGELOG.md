...
### What Changed in CladeTime 0.4.0

The CladeTime package (v0.4.0) now includes automatic fallback to variant-nowcast-hub archives when Nextstrain S3 metadata is unavailable for historical dates. This resolves the issue where `get_target_data.py` workflow failures occurred due to Nextstrain's October 2025 cleanup of historical `metadata_version.json` files.

**IMPORTANT BREAKING CHANGES IN 0.4.0:**
- **Minimum `sequence_as_of` date**: Changed from 2023-05-01 to **2025-09-29** (due to Nextstrain's 90-day S3 retention policy)
- **Minimum `tree_as_of` date**: Enforced at 2024-10-09 (via variant-nowcast-hub archives)
- **Error handling**: CladeTime now raises `CladeTimeDataUnavailableError` for dates outside availability windows instead of silently defaulting
- **Test infrastructure**: Updated to handle Nextstrain's October 2025 S3 cleanup

These breaking changes **do NOT affect variant-nowcast-hub workflows** because VNH always uses current dates (via `CladeTime()` with no parameters), which are well within the data availability window.
...
- The changes are **fully backwards compatible for VNH workflows**
- No modifications needed to `get_target_data.py`, `get_clades_to_model.py`, or other scripts
- VNH workflows always use current dates, which are within the new data availability window (>= 2025-09-29)
- Historical date access (before 2025-09-29) now requires fallback to Hub archives and will only work with CladeTime v0.4.0 or later
...
## Summary for variant-nowcast-hub PR #805

**CladeTime 0.4.0 Changes:**
- ✅ Hub fallback mechanism implemented (fetches from VNH's `auxiliary-data/modeled-clades/`)
- ✅ Breaking change: Minimum `sequence_as_of` date now 2025-09-29 (90-day S3 retention)
- ✅ New exception: `CladeTimeDataUnavailableError` for unavailable dates
- ✅ Comprehensive test coverage and documentation

**VNH Compatibility:**
- ✅ **Fully compatible** - VNH workflows use current dates (always >= 2025-09-29)
- ✅ No code changes needed in VNH scripts
- ✅ Dependency updated to `cladetime>=0.4.0,<0.5.0` in this PR
- ✅ Tested successfully with VNH's `get_clades_to_model.py`

**Next Steps:**
1. Merge CladeTime PR #181 and release 0.4.0 to PyPI
2. Merge this VNH PR #805 after CladeTime release
3. Re-run failed `run-post-submission-jobs.yaml` workflows to verify fix
